...
def batch_greedy_decode(model, encoder_batch_data, vocab, params):
    ...
    for t in range(params['max_dec_len']):
        ...
        # predicted_ids = tf.math.top_k(final_dists, k=4)[1][:, -1]
...
def greedy_decode(model, dataset, vocab, params):
    ...
    #     enc_padding_mask, combined_mask, dec_padding_mask = create_masks(encoder_batch_data['enc_input'],
    #                                                                      decoder_batch_data['dec_input'])
    #
    #     outputs = model(encoder_batch_data['enc_input'],
    ...
    #     for sent_idxs in pred_idx.numpy():
    #         sent = [vocab.id_to_word(idx) for idx in sent_idxs]
    #         sent = ' '.join(sent)
    #         results.append(sent)
    #         print(sent)
    #
